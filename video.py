    ...
    if(args.pb != ''):
        ...
        with tf.Session(graph=frozenGraph, config=config) as sess:
            ...
            while(True):
                ...
                fps = int(1/(end-start))
                img = cv2.putText(img, 'FPS:{} one_frame_processing_time:%.3f'.format(fps)%(end-start), (100,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (217,255,128), 2)
        ...
        with tf.Session(config=config) as sess:
            ...
            while(True):
                ...
                fps = int(1/(end-start))
                img = cv2.putText(img, 'FPS:{} one_frame_processing_time:%.3f'.format(fps)%(end-start), (100,50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,128), 1)
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                cv2.imshow('冲',img)
                cv2.waitKey(1)
